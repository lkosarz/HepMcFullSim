...
srun -n $SLURM_JOB_NUM_NODES --ntasks-per-node=1 cp -r $HOME/eic-container/eic_xl_24.11.1-stable.sif $TMPDIR/local/lib/
ls
ls $TMPDIR/local/
ls $TMPDIR/local/*
#./eic-shell -- ./runBatch_SLURM.sh $SLURM_ARRAY_TASK_ID
#./eic-shell -- ./runBatch_SLURM.sh
apptainer run $TMPDIR/local/lib/eic_xl_24.11.1-stable.sif -- ./runBatch_SLURM.sh ${SLURM_ARRAY_TASK_ID} $TMPDIR/data/files.list
